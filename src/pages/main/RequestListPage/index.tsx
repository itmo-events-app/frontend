...
import {
  RegistrationRequestForAdmin,
  RegistrationRequestForAdminStatusEnum,
} from '@shared/api/generated/model/registration-request-for-admin.ts';
...
class Request {
  ...
  constructor(req: RegistrationRequestForAdmin) {
    ...
    switch (req.status || RegistrationRequestForAdminStatusEnum.New) {
      case RegistrationRequestForAdminStatusEnum.New:
        this.status = RequestStatus.New;
        break;
      case RegistrationRequestForAdminStatusEnum.Approved:
        this.status = RequestStatus.Approved;
        break;
      case RegistrationRequestForAdminStatusEnum.Declined:
...
export default function RequestListPage() {
  ...
  function _renderButtons(req: Request) {
    if (req.status == RequestStatus.New) {
      return (
        <>
          <Button className={styles.button} onClick={() => _approveRequestClick(req)}>
            Утвердить
          </Button>
          <Button
            className={appendClassName(styles.button, styles.decline_button)}
            onClick={() => _declineRequestClick(req)}
          >
            Отклонить
          </Button>
        </>
    ...
    return (
      <div className={styles.request_entry} key={req.id}>
        <div className={styles.request_header}>
          ...
          <div className={styles.button_panel}>{_renderButtons(req)}</div>
        </div>
      </div>
    );
  }
  ...
  const _renderedRequests: any[] = requests.map((req: Request) => {
    return new PageEntry(() => {
      return _renderRequestEntry(req);
    });
  });
      ...
      bottomRight={
        requests.length == 0 ? (
          <Content>
            ...
            <Button className={styles.button_reload} onClick={_fetchData}>
              Перезагрузить
            </Button>
          </Content>
        ) : (
