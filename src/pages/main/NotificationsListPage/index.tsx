...
class Notification {
  ...
  constructor(eventName: string, activityName: string, taskName: string, date: string, time: string, text: string) {
    this.id = uid();
...
const _notifications: Notification[] = [
  ...
  new Notification("At vero eos",
    ...
    "31.10.2008",
    "20:31",
    "Quis autem vel eum iure reprehenderit, quis nostrum exercitationem ullam corporis suscipit laboriosam, obcaecati cupiditate non provident, similique sunt in culpa, qui officia deserunt mollitia animi, id est laborum et dolorum fuga."),new Notification("At vero eos",
    "et accusamus",
    "et iusto",
    "31.10.2008",
    "20:31",
...
export default function NotificationsListPage() {
  ...
  const _deleteNotification = (notification: Notification) => {
  ...
  function _renderNotificationEntry(ne: NotificationEntry) {
    return (
      <div className={styles.notification_entry} key={ne.data.id}>
        ...
        <div className={styles.notification_delete}>
          <Button className={styles.delete_button} onClick={() => _deleteEntryClick(ne)}>Удалить</Button>
        </div>
      </div>
    );
  }
  ...
  function _deleteEntryClick(notificationEntry: NotificationEntry) {
    // some callback to server?
    _deleteNotification(notificationEntry.data)

    setNotifications(notifications.filter(n => n.id !== notificationEntry.data.id))
  }
