...
class Notification {
  ...
  constructor(notificationResponse: NotificationResponse) {
    if (
      notificationResponse.id === undefined ||
      ...
      console.warn(
        `One of the fields of NotificationResponse is unidentified - possible errors in render`,
...
export default function NotificationListPage() {
  ...
  const _updateNotifications = () => {
    api.notification
      .getNotifications(page - 1, pageSize)
      .then((result) => {
  ...
  const _readNotification = (notification: Notification) => {
    api.notification
      .setOneAsSeenNotification(notification.id)
    ...
    return (
      <div className={styles.notification_entry} key={ne.data.id}>
        <div className={styles.notification_header}>
          <span
            className={styles.notification_titles}
            onClick={() => {
        ...
        <div className={styles.notification_read}>
          <Button className={ne.data.seen ? styles.read_button : styles.not_read_button} onClick={() => _readEntry(ne)}>
      ...
      bottomRight={
        <Content>
          ...
          <div className={styles.read_all_button_div}>
            <Button
              className={styles.not_read_button + styles.read_all_button}
