...
const CreatePlaceDialog = ({ onClose }: { onClose: () => void }) => {
  ...
  const createPlace = () => {
    ...
    placeService
      .createPlace(api, placeName, address, formatEnum[format.value], roomName, description, latitude, longitude)
      .then(() => onClose());
  };
      ...
      <Dialog className={styles.dialog_content} text={'Создание площадки'}>
        <div onClick={(e) => e.stopPropagation()}>
          <div className={styles.place_form}>
            ...
            <iframe
              id="itmo-map-iframe"
              src={(window as any).ENV_GEO_URL + '/map.html?noscroll&select_only_areas'}
              width="100%"
              height="420px"
            ></iframe>
        ...
        <a className={styles.place_entry}>
          ...
          <div className={styles.place_buttons}>
            {hasAnyPrivilege(
              ...
              new Set([new PrivilegeData(PrivilegeNames.CREATE_EVENT_VENUE)])
            ) ? (
            ...
            {hasAnyPrivilege(
              privilegeContext.systemPrivileges,
              new Set([new PrivilegeData(PrivilegeNames.DELETE_EVENT_VENUE)])
            ) ? (
              <div>
                <Button
                ...
                >
                  <svg
                    className={styles.delete_button_svg}
