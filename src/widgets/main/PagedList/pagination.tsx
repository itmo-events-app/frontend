...
function Pagination(props: Props) {
  ...
  const renderPagination = () => {
    ...
    pages.push(
      <a
        key={1}
        className={`${styles.nav_page_num} ${1 === props.pageProps.page ? styles.nav_page_num_selected2 : ''}`}
        onClick={() => _handlePageClick(1)}
      >
        1
      </a>
    );
    ...
    if (visiblePages[0] > 2) {
      pages.push(
        <span className={styles.nav_page_num} key="ellipsis1">
          ...
        </span>
      );
    }
    visiblePages.forEach((page) => {
      pages.push(
        <a
          key={page}
          className={`${styles.nav_page_num} ${page === props.pageProps.page ? styles.nav_page_num_selected2 : ''}`}
          onClick={() => _handlePageClick(page)}
        >
          {page}
        </a>
      );
    ...
    if (visiblePages[visiblePages.length - 1] < totalPages - 1) {
      pages.push(
        <span className={styles.nav_page_num} key="ellipsis2">
          ...
        </span>
      );
    }
    if (totalPages > 1) {
      pages.push(
        <a
          key={totalPages}
          className={`${styles.nav_page_num} ${totalPages === props.pageProps.page ? styles.nav_page_num_selected2 : ''}`}
          onClick={() => _handlePageClick(totalPages)}
        >
          {totalPages}
        </a>
      );
  ...
  const renderItems = (listWrapper?: JSX.Element) => {
    ...
    if (listWrapper === null || listWrapper === undefined) return <div className={styles.list}>{rendererdItems}</div>;
  ...
  function _navInfo() {
    return (
      <div className={styles.nav_page_nums}>
        <div className={styles.nav_page_num_selected}>{(props.pageProps.page - 1) * props.pageProps.size + 1}</div>
        ...
        <div className={styles.nav_page_num}>{props.pageProps.total}</div>
    ...
    <div className={styles.list_container}>
      <div className={styles.navigator}>
        ...
        <div className={styles.nav_buttons}>
          ...
          <div className={styles.nav_page_nums}>{renderPagination()}</div>
          <a className={styles.nav_button} onClick={() => _handlePageClick(props.pageProps.page + 1)}>
            <ArrowRight className={styles.arrow} />
          </a>
        </div>
        <div className={styles.nav_buttons}>
          ...
          <div className={styles.nav_page_num}>{props.pageProps.size}</div>
          <a className={styles.nav_button} onClick={() => _handleSizeClick(props.pageProps.size + 1)}>
            <ArrowRight className={styles.arrow} />
          </a>
        </div>
