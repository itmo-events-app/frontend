...
function PageTabs(props: Props) {
  ...
  function _createUnselectedTab(tab: PageTab) {
    return (
      <div key={tab.id} className={styles.page_tab_container + ' ' + (props.className ?? '')} onClick={_select(tab)}>
        <a href="#" className={styles.page_tab} onClick={_select(tab)}>
          {tab.text}
        </a>
      </div>
    );
  ...
  function _createSelectedTab(tab: PageTab) {
    return (
      <div
        key={tab.id}
        className={styles.page_tab_container + ' ' + styles.page_tab_container_selected + ' ' + (props.className ?? '')}
        onClick={_select(tab)}
      >
        <a href="#" className={styles.page_tab_selected} onClick={_select(tab)}>
          {tab.text}
        </a>
      </div>
    );
  ...
  return <div className={styles.page_tabs}>{_createTabList(props.items)}</div>;
}
